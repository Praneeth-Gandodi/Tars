...
## Persists a compaction summary as a role='system' message with summary_flag=1,
## so it appears in history but is not counted as a regular user/assistant message.
def save_summary(text: str, session_id: int, model_id: int = None):
    """
    Save a compaction summary linked to the session.
    Args:
        text (str): The summary content generated by the LLM.
        session_id (int): The session the summary belongs to.
        model_id (int, optional): Model that created the summary.
    Returns:
        cid (int): The row id of the saved summary, or None on failure.
    """
    conn = get_connection()
    cursor = conn.cursor()
    try:
        cursor.execute(
            """
            INSERT INTO conversations (session_id, role, content, model_id, summary_flag)
            VALUES (?, 'system', ?, ?, 1)
            """,
            (session_id, text, model_id)
        )
...
def get_last_messages(limit: int = 20, session_id: int = None):
    """
    Returns the last N messages given by timestamp.
    Args:
        limit (int, optional): No of previous message you Need. Defaults to 20.
        session_id (int, optional): If given, only messages from that session are returned.
    """
    conn = get_connection()
    cursor = conn.cursor()
